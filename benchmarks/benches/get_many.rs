...
fn benchmark_getting_subset_of_inserted_addresses(
		...
		let temp = TempDir::new(
				format!(
						"benchmark_paritydb_get_{}_from_{}_addresses",
						get_subset_count,
						insert_count
						).as_str())
				.unwrap();
...
#[bench]
fn paritydb_from_1_inserted_get_1_addresses(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 1, 1, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_1000_inserted_get_random_of_1000_addresses(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 1000, 1000, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_100000_inserted_get_1_address(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 100000, 1, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_100000_inserted_get_random_of_1000_addresses(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 100000, 1000, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_100000_inserted_get_random_of_100000_addresses(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 100000, 100000, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_1000000_inserted_get_1_address(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 1000000, 1, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_1000000_inserted_get_random_of_1000_addresses(b: &mut Bencher) {
		benchmark_getting_subset_of_inserted_addresses(b, 1000000, 1000, LARGE_KEY_INDEX_BITS);
}

#[bench]
fn paritydb_from_1000000_inserted_get_random_of_1000000_addresses(b: &mut Bencher) {
