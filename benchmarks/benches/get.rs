...
#[bench]
fn paritydb_dummy_get(b: &mut Bencher) {
	use paritydb::{Database, ValuesLen, Options, Transaction};

	let temp = TempDir::new("paritydb_dummy_get").unwrap();
...
#[bench]
fn paritydb_dummy_get_null(b: &mut Bencher) {
	use paritydb::{Database, ValuesLen, Options};

	let temp = TempDir::new("paritydb_dummy_get_null").unwrap();
